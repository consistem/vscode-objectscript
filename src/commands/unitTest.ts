...
function findMethodItemByLegacyName(
...
function deriveMethodResultsFromClassSummary(
  ...
  const hasSpecificFailures = Array.from(summaries.values()).some(
    (summary) => summary.status === TestStatus.Failed
  );

  const fallbackMethods =
    requestedMethods && requestedMethods.size
      ? Array.from(requestedMethods)
      : availableMethods;
...
function applyTestResultDecorations(
  ...
  for (const { item, result, durationText } of methodResults) {
    const editor = vscode.window.visibleTextEditors.find(
      (e) => e.document.uri.toString() === item.uri.toString()
    );
...
function resolveRelativeRootUris(
...
function pathMatchesAnyWorkspaceRoot(
  workspaceRootItem: vscode.TestItem,
...
async function executeLegacyRunner(
  ...
  try {
    ...
    const consoleByMethod = groupConsoleByMethod(
      Array.isArray(response.console) ? response.console : []
    );
    ...
    for (const testResult of response.results) {
      ...
      if (testResult.method) {
        ...
        const derivedSummaries = deriveMethodResultsFromClassSummary(
          testResult,
          requestedMethods,
          availableMethods
        );
