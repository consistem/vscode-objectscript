...
| `objectscript.ccs.requestTimeout`         | `number`                  | `5000`         | Tempo limite (ms) aplicado às chamadas HTTP rápidas do módulo (definição, documentação, gatilhos etc.). Valores negativos/inválidos são normalizados; `0` desativa o tempo limite. Não se aplica à conversão de item nem à análise de versões do item. |
...
| `consistem.analisarVersaoItem.timeout`               | `number`                  | `120000`       | Tempo limite (ms) exclusivo do comando `Versões do Item - Analisar`. Aumente nas configurações de usuário em servidores/máquinas lentas; `0` desativa o tempo limite. |
